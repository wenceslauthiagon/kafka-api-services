...
import {
  GetUserPinAttemptsByUserUseCase as UseCase,
...
describe('GetUserPinAttemptsByUserUseCase', () => {
  ...
  it('TC0001 - Should get userPinAttempts successfully', async () => {
  ...
  it('TC0002 - Should get with not existubst userPinAttempts', async () => {
    ...
    expect(foundUserPinAttempts.attempts).toBe(0);
    expect(foundUserPinAttempts.user.id).toBe(user.id);

    expect(mockUpdatedUserPinAttemptsEvent).toHaveBeenCalledTimes(1);
  });

  it('TC0003 - Should not get with not valid uuid', async () => {
    const usecase = new UseCase(
      logger,
      userRepository,
      userPinAttemptsRepository,
  ...
  it('TC0004 - Should not get with not valid found user', async () => {
    const usecase = new UseCase(
      logger,
      userRepository,
      userPinAttemptsRepository,
    ...
    await expect(() =>
      usecase.execute(new UserEntity({ uuid: uuidV4() })),
