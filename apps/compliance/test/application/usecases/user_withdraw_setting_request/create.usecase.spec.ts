...
import {
  CreateUserWithdrawSettingRequestUseCase as UseCase,
  OperationService,
...
describe('CreateUserWithdrawSettingRequestUseCase', () => {
  const mockEmitter = () => {
    ...
    const mockPendingUserWithdrawSettingRequestEvent: jest.Mock = On(
      eventEmitter,
    ).get(method((mock) => mock.pending));

    return {
      eventEmitter,
      mockPendingUserWithdrawSettingRequestEvent,
    };
  };
  ...
  const makeSut = () => {
    ...
    const { eventEmitter, mockPendingUserWithdrawSettingRequestEvent } =
      mockEmitter();
  ...
  describe('With invalid parameters', () => {
    it('TC0001 - Should not create if missing params', async () => {
      ...
      expect(mockPendingUserWithdrawSettingRequestEvent).toHaveBeenCalledTimes(
        0,
      );
