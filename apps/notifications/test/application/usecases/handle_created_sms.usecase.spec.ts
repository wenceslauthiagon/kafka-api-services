...
describe('Test handle created sms use case', () => {
  ...
  const executeUseCase = async (id: string) => {
    ...
    const handleSentSmsUseCase = new HandleSmsCreatedUseCase(
      smsRepository,
      smsEventEmitter,
      mockSmsGateway,
      logger,
    ...
    const handledSms = await handleSentSmsUseCase.execute(id);
  ...
  it('TC0001 - Should handle sent SMS successfully', async () => {
    ...
    expect(mockSendSmsGateway.mock.calls[0][0].id).toBe(sentSms.id);
  });
