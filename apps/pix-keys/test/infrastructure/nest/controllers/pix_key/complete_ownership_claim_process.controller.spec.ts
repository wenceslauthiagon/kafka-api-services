...
import {
  PixKeyModel,
  CompleteOwnershipClaimProcessMicroserviceController as Controller,
...
describe('CompleteOwnershipClaimProcessMicroserviceController', () => {
  ...
  describe('CompleteOwnershipClaimProcess', () => {
    describe('With valid parameters', () => {
      it('TC0001 - Should ready process successfully', async () => {
        ...
        const message: CompleteOwnershipClaimProcessRequest = { key };
        ...
        expect(mockEmitPixKeyEvent.mock.calls[0][0]).toBe(
          PixKeyEventType.OWNERSHIP_READY,
        );
      });

      it('TC0002 - Should complete ownership process with already ownership ready key successfully', async () => {
        const claim = await PixKeyClaimFactory.create<PixKeyClaimModel>(
          PixKeyClaimModel.name,
        );
