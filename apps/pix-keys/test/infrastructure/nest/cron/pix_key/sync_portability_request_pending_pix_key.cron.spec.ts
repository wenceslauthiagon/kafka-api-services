...
import {
  PixKeyModel,
  SyncPortabilityRequestPendingPixKeyCronService as Cron,
  KAFKA_EVENTS,
  PixKeyClaimModel,
...
describe('SyncClaimPendingExpiredPixKeyCronService', () => {
  ...
  describe('With invalid parameters', () => {
    it('TC0001 - Should not sync portability request pending if is not expired', async () => {
      ...
      expect(mockEmitKafkaService).not.toHaveBeenCalledWith(
        KAFKA_EVENTS.KEY.PORTABILITY_REQUEST_CONFIRM_OPENED,
    ...
    it('TC0002 - Should not sync claim portability request pending if has invalid state', async () => {
      const pixKey = await PixKeyFactory.create<PixKeyModel>(PixKeyModel.name, {
      ...
      expect(result.state).not.toBe(
        KeyState.PORTABILITY_REQUEST_CONFIRM_OPENED,
      );
    });
  });
  ...
  describe('With valid parameters', () => {
    it('TC0003 - Should execute successfully', async () => {
      ...
      expect(result.id).toBe(pixKey.id);
      expect(result.state).toBe(KeyState.PORTABILITY_REQUEST_CONFIRM_OPENED);
