...
export class HandlePortabilityOpenedFailedPixKeyEventUseCase {
  /**
   * Default constructor.
  ...
  constructor(
    ...
    this.logger = logger.child({
      context: HandlePortabilityOpenedFailedPixKeyEventUseCase.name,
    });
  }
  ...
  async execute(id: string): Promise<PixKey> {
    ...
    // Indeponent retry
    if ([KeyState.PORTABILITY_STARTED, KeyState.ERROR].includes(pixKey.state)) {
      return pixKey;
    }
    ...
    this.logger.debug('Start portability process failed.', { pixKey });
