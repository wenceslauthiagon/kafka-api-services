...
export class HandleOwnershipPendingExpiredPixKeyUseCase {
  /**
   * Default constructor.
  ...
  constructor(
    ...
    this.logger = logger.child({
      context: HandleOwnershipPendingExpiredPixKeyUseCase.name,
    });
  }

  /**
   * Update ownership pending key to canceled.
   *
   * @param id The key id
   * @returns Key updated.
  ...
  async execute(id: string): Promise<PixKey> {
    ...
    // Indeponent retry
    if (!pixKey || pixKey.state !== KeyState.OWNERSHIP_PENDING) {
      return pixKey;
    }
