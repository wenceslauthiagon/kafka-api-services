...
export class CompleteOwnershipClaimProcessUseCase {
  constructor(
    ...
    this.logger = logger.child({
      context: CompleteOwnershipClaimProcessUseCase.name,
    });
  }
  ...
  async execute(key: string): Promise<PixKey> {
    ...
    if (pixKey.state === KeyState.OWNERSHIP_READY) {
      return pixKey;
    }
    ...
    pixKey.state = KeyState.OWNERSHIP_READY;
    ...
    // Fire event.
    this.eventEmitter.ownershipReadyPixKey(pixKey);

    this.logger.debug('Ownership ready.', { pixKey });
