...
import { UpdatedExchangeContractParams } from '@zro/otc/application';
...
describe('ExchangeContractRestController (e2e)', () => {
  ...
  describe('/otc/exchange-contracts/:id (PATCH)', () => {
    it('TC0001 - Should patch successfully', async () => {
      ...
      const payload: UpdatedExchangeContractParams = {
        contractNumber: '123',
        vetQuote: 456,
      };

      const res = await request(app.getHttpServer())
        .patch(`/otc/exchange-contracts/${exchangeContract.id}`)
        .send(payload)
