...
describe('DeleteUserWalletUseCase', () => {
  ...
  describe('With valid parameters', () => {
    it('TC0001 - Should delete user wallet successfully ', async () => {
      ...
      mockGetByUserAndWalletRepository.mockResolvedValueOnce({ wallet, user });

      await sut.execute(owner, user, wallet);
  ...
  describe('With invalid parameters', () => {
    it('TC0002 - Should not delete user wallet if missing data', async () => {
      ...
      const tests = [
        ...
        () => sut.execute(owner, null, null),
        () => sut.execute(null, user, null),
        () => sut.execute(null, null, wallet),
        () => sut.execute(owner, user, null),
        () => sut.execute(null, user, wallet),
        () => sut.execute(owner, null, wallet),
        () => sut.execute(owner, user, new WalletEntity({})),
        () => sut.execute(owner, new UserEntity({}), wallet),
        () => sut.execute(new UserEntity({}), user, wallet),
      ];
    ...
    it('TC0004 - Should not delete user wallet if user received is owner', async () => {
      ...
      const testScript = () => sut.execute(owner, owner, wallet);
    ...
    it('TC0005 - Should not delete user wallet if not found', async () => {
      ...
      mockGetByUuidWalletRepository.mockResolvedValueOnce(wallet);
      mockGetByUserAndWalletRepository.mockResolvedValueOnce(null);
