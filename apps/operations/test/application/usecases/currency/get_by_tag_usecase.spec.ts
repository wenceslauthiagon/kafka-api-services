...
import {
  GetCurrencyByTagUseCase as UseCase,
...
describe('GetCurrencyByTagUseCase', () => {
  ...
  describe('With valid parameters', () => {
    it('TC0002 - Should get currency by tag successfully', async () => {
      ...
      const result = await usecase.execute(currency.tag);

      expect(result).toBeDefined();
      expect(result.tag).toBe(currency.tag);
    });

    it('TC0003 - Should not get currency with incorrect tag', async () => {
      const tag = uuidV4();

      const usecase = new UseCase(logger, currencyRepository);

      const testScript = () => usecase.execute(tag);
