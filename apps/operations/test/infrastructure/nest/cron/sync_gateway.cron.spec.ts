...
import {
  SyncGatewayReportsCronService as Cron,
  CurrencyModel,
  OperationModel,
  SyncGatewayReportsCronConfig,
...
describe('SyncGatewayReportsCronService', () => {
  ...
  beforeAll(async () => {
    ...
    const configService: ConfigService<SyncGatewayReportsCronConfig> =
    ...
    it('TC0001 - Should execute successfully for withdrawal', async () => {
      ...
      const operation = await OperationFactory.create<OperationModel>(
        OperationModel.name,
        {
          ...
          createdAt: now,
          chargeback: null,
          description: 'Gateway: withdrawal',
        },
      );
