...
import {
  NotifyConfirmBankingTedControllerEvent,
  NotifyConfirmBankingTedEventEmitterControllerInterface,
  NotifyConfirmBankingTedEventType,
} from '@zro/api-topazio/interface';

const eventMapper = KAFKA_EVENTS.NOTIFY_CONFIRM_BANKING_TED;

type NotifyConfirmBankingTedKafkaEvent =
  KafkaMessage<NotifyConfirmBankingTedControllerEvent>;

/**
 * ApiTopazio microservice.
 */
@KafkaCreateEvent(Object.values(eventMapper))
export class NotifyConfirmBankingTedEventKafkaEmitter
  implements NotifyConfirmBankingTedEventEmitterControllerInterface
{
  /**
   * Default constructor.
  ...
  constructor(
    ...
    this.logger = logger.child({
      context: NotifyConfirmBankingTedEventKafkaEmitter.name,
    });
  }
  ...
  emitConfirmEvent(
    ...
    const data: NotifyConfirmBankingTedKafkaEvent = {
      key: `${this.requestId}`,
