...
@Controller('notify-credit')
export class NotifyCreditRestController {
  /**
   * Default constructor.
  ...
  constructor(
    ...
    this.logger = logger.child({
      context: NotifyCreditRestController.name,
    });
  }
  ...
  async execute(
    @Body() data: NotifyCreditBody,
    @RequestId() requestId: string,
  ): Promise<void> {
    const logger = this.logger.child({ loggerId: requestId });

    // Create a payload.
    const payload = new NotifyCreditEntity(data);
    logger.debug('Notify credit in topazio.', { payload });

    // Call create pixKey service.
    await this.service.notifyCredit(requestId, payload);
