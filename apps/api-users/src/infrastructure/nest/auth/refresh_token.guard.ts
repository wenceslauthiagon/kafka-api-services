...
export class RefreshTokenGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!headers?.['x-access-token']) {
      logger.info('Do not validate without header access token.');
    ...
    try {
      verifiedToken = this.jwtService.verify<AccessToken>(token, {
        ignoreExpiration: true,
      });
    } catch (error) {
      logger.info('Do not validate an invalid access token.', error);
      throw new UnauthorizedException();
    }

    // Check if refresh token exists.
    const cached = await this.redisService.get<RefreshTokenCache>(
      `refresh-token-users-${verifiedToken.id}`,
    );

    if (
      cached?.data?.refreshToken?.uuid === verifiedToken?.refresh_token?.uuid
    ) {
      logger.info('Validate refresh token user authorized.');
      request.user = cached.data.user;
      return true;
    }
