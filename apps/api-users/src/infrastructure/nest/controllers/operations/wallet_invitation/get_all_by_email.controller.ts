...
import {
  GetAllWalletInvitationByEmailResponseItem,
  GetAllWalletInvitationByEmailResponse,
  GetAllWalletInvitationByEmailRequest,
  GetAllWalletInvitationByEmailRequestSort,
} from '@zro/operations/interface';
import { AuthUserParam } from '@zro/users/infrastructure';
import { GetAllWalletInvitationByEmailServiceKafka } from '@zro/operations/infrastructure';

class GetAllWalletInvitationByEmailParams extends PaginationParams {
  @ApiPropertyOptional({
    description: 'Page sort attribute.',
    enum: GetAllWalletInvitationByEmailRequestSort,
  })
  @IsOptional()
...
class GetAllWalletInvitationByEmailRestResponseItem {
  @ApiProperty({
  ...
  constructor(props: GetAllWalletInvitationByEmailResponseItem) {
    this.id = props.id;
...
class GetAllWalletInvitationByEmailRestResponse extends PaginationRestResponse {
  @ApiProperty({
    description: 'WalletInvitations data.',
    type: [GetAllWalletInvitationByEmailRestResponseItem],
  })
  data!: GetAllWalletInvitationByEmailRestResponseItem[];

  constructor(props: GetAllWalletInvitationByEmailResponse) {
    super(props);
    this.data = props.data.map(
      (item) => new GetAllWalletInvitationByEmailRestResponseItem(item),
    );
  }
}
...
@Controller('operations/wallet-invitations/my')
@DefaultApiHeaders()
@ApiBearerAuth()
@HasPermission('api-users-get-operations-wallet-invitations-my')
export class GetAllWalletInvitationByEmailRestController {
  /**
   * get walletInvitations endpoint.
   */
  @ApiOperation({
    summary: "List user's received wallet invitations.",
    description:
  ...
  @ApiOkResponse({
    ...
    type: GetAllWalletInvitationByEmailRestResponse,
  })
  @ApiUnauthorizedResponse({
  ...
  async execute(
    ...
    @Query() query: GetAllWalletInvitationByEmailParams,
    @KafkaServiceParam(GetAllWalletInvitationByEmailServiceKafka)
    getAllWalletInvitationByEmailService: GetAllWalletInvitationByEmailServiceKafka,
    @LoggerParam(GetAllWalletInvitationByEmailRestController)
    logger: Logger,
  ): Promise<GetAllWalletInvitationByEmailRestResponse> {
    // GetAll payload.
    const payload: GetAllWalletInvitationByEmailRequest = {
      email: user.email,
      state: query.state,
    ...
    logger.debug('GetAll wallet invitations.', { user, payload });

    // Call get all walletAccount service.
    const result = await getAllWalletInvitationByEmailService.execute(payload);

    logger.debug('Wallet invitations found.', { result });

    const response = new GetAllWalletInvitationByEmailRestResponse(result);
