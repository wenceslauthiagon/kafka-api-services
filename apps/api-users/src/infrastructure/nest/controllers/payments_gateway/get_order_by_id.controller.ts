...
import { GetOrderByIdServiceKafka } from '@zro/payments-gateway/infrastructure';
import {
  GetOrderByIdRequest,
...
export class GetOrderByIdRestResponse {
  @ApiProperty({
    description: 'Order ID',
    example: 1,
  })
  ...
  constructor(props: GetOrderByIdResponse) {
    this.id = props.id;
...
/**
 * GetOrderById controller. Controller is protected by JWT access token.
 */
@ApiTags('Payments Gateway | Orders')
@Controller('payments-gateway/orders/:id')
@DefaultApiHeaders()
@ApiBearerAuth()
@WalletApiHeader()
@HasPermission('api-users-get-payments-gateway-orders-by-id')
export class GetOrderByIdRestController {
  /**
  ...
  @ApiOkResponse({
    description: 'OrderById found successfully.',
    type: GetOrderByIdRestResponse,
  })
  @ApiUnauthorizedResponse({
  ...
  async execute(
    ...
    @KafkaServiceParam(GetOrderByIdServiceKafka)
    service: GetOrderByIdServiceKafka,
    @LoggerParam(GetOrderByIdRestController)
    logger: Logger,
  ): Promise<GetOrderByIdRestResponse> {
    // Creates a payload
    const payload: GetOrderByIdRequest = {
      wallet_id: wallet.id,
      id: params.id,
    };
    ...
    const response = result && new GetOrderByIdRestResponse(result);
