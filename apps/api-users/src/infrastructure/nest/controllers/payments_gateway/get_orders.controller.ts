...
import { GetOrdersServiceKafka } from '@zro/payments-gateway/infrastructure';
import {
  GetOrdersRequest,
  GetOrdersResponse,
  TCompany,
...
export class GetOrdersRestResponseItem {
  @ApiProperty({
...
export class GetOrdersRestResponse {
  @ApiProperty({
    description: 'Orders found.',
    example: [GetOrdersRestResponseItem],
  })
  data!: GetOrdersRestResponseItem[];
  ...
  constructor(props: GetOrdersResponse) {
 ...
 */
@ApiTags('Payments Gateway | Orders')
@Controller('payments-gateway/orders')
@DefaultApiHeaders()
@ApiBearerAuth()
@WalletApiHeader()
@HasPermission('api-users-get-payments-gateway-orders')
export class GetOrdersRestController {
  /**
   * Get orders endpoint.
   */
  @ApiOperation({
  ...
  async execute(
    ...
    @KafkaServiceParam(GetOrdersServiceKafka)
    service: GetOrdersServiceKafka,
    @LoggerParam(GetOrdersRestController)
    logger: Logger,
  ): Promise<GetOrdersRestResponse> {
    // Creates a payload
    ...
    logger.debug('Get orders.', { user, wallet, payload });

    // Call orders service.
    const result = await service.execute(payload);

    logger.debug('Found orders.', { result });

    const response = result && new GetOrdersRestResponse(result);
