...
import { WithdrawalByQrCodeDynamicPaymentServiceKafka } from '@zro/pix-payments/infrastructure';
import {
...
export class PaymentByWithdrawalQrCodeDynamicBody extends PinBody {
  @ApiProperty({
...
export class WithdrawalByQrCodeDynamicPaymentRestResponse {
  @ApiProperty({
  ...
  constructor(props: WithdrawalByQrCodeDynamicPaymentResponse) {
    this.id = props.id;
...
@TransactionApiHeader()
@EnableReplayProtection()
@Controller('pix/payments/by-qr-code/dynamic/withdrawal')
@HasPermission('api-users-post-pix-payments-by-qr-code-dynamic-withdrawal')
export class WithdrawalByQrCodeDynamicPaymentRestController {
  /**
   * send payment endpoint.
   */
  @ApiOperation({
    summary: 'Withdrawal a payment with qr code dynamic.',
  })
  @ApiCreatedResponse({
    description: 'Payment accomplished.',
    type: WithdrawalByQrCodeDynamicPaymentRestResponse,
  })
  @ApiUnauthorizedResponse({
  ...
  async execute(
    ...
    @Body() body: PaymentByWithdrawalQrCodeDynamicBody,
    @KafkaServiceParam(WithdrawalByQrCodeDynamicPaymentServiceKafka)
    service: WithdrawalByQrCodeDynamicPaymentServiceKafka,
    @LoggerParam(WithdrawalByQrCodeDynamicPaymentRestController)
    logger: Logger,
    @RequestTransactionId() transactionId: string,
  ): Promise<WithdrawalByQrCodeDynamicPaymentRestResponse> {
    // Send a payload.
    const payload: WithdrawalByQrCodeDynamicPaymentRequest = {
      id: transactionId,
      userId: user.uuid,
      walletId: wallet.id,
    ...
    const response = new WithdrawalByQrCodeDynamicPaymentRestResponse(result);
