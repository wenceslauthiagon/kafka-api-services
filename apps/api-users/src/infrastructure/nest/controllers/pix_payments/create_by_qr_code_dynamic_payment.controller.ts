...
import { CreateByQrCodeDynamicPaymentServiceKafka } from '@zro/pix-payments/infrastructure';
import { PinGuard, PinBody } from '@zro/api-users/infrastructure';

class PaymentByQrCodeDynamicBody extends PinBody {
  @ApiProperty({
...
class CreateByQrCodeDynamicPaymentRestResponse {
  @ApiProperty({
  ...
  constructor(props: CreateByQrCodeDynamicPaymentResponse) {
    this.id = props.id;
...
@TransactionApiHeader()
@EnableReplayProtection()
@Controller('pix/payments/by-qr-code/dynamic/instant-billing')
@HasPermission('api-users-post-pix-payments-by-qr-code-dynamic-instant-billing')
export class CreateByQrCodeDynamicPaymentRestController {
  /**
   * send payment endpoint.
   */
  @ApiOperation({
    summary: 'Create a payment with qr code dynamic.',
  })
  @ApiCreatedResponse({
    description: 'Payment accomplished.',
    type: CreateByQrCodeDynamicPaymentRestResponse,
  })
  @ApiUnauthorizedResponse({
  ...
  async execute(
    ...
    @Body() body: PaymentByQrCodeDynamicBody,
    @KafkaServiceParam(CreateByQrCodeDynamicPaymentServiceKafka)
    service: CreateByQrCodeDynamicPaymentServiceKafka,
    @LoggerParam(CreateByQrCodeDynamicPaymentRestController)
    logger: Logger,
    @RequestTransactionId() transactionId: string,
  ): Promise<CreateByQrCodeDynamicPaymentRestResponse> {
    // Send a payload.
    const payload: CreateByQrCodeDynamicPaymentRequest = {
      id: transactionId,
      userId: user.uuid,
      walletId: wallet.id,
    ...
    const response = new CreateByQrCodeDynamicPaymentRestResponse(result);
