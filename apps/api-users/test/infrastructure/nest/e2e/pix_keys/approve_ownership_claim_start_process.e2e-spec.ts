...
describe('PixKeyController (e2e)', () => {
  ...
  describe('/pix/keys/id/claims/start (POST)', () => {
    it('TC0001 - Should start ownership process successfully', async () => {
      ...
      const res = await request(app.getHttpServer())
        .post(`/pix/keys/${pixKey.id}/claims/start`)
      ...
      expect(res.body.data.state).toBe(KeyState.OWNERSHIP_OPENED);
