...
describe('PixKeyController (e2e)', () => {
  ...
  describe('/pix/keys/id/portabilities/approve (POST)', () => {
    it('TC0001 - Should post successfully', async () => {
      ...
      const res = await request(app.getHttpServer())
        .post(`/pix/keys/${pixKey.id}/portabilities/approve`)
      ...
      expect(res.body.data.state).toBe(
    ...
    it('TC0002 - Should fail with invalid id', async () => {
      ...
      const res = await request(app.getHttpServer())
        .post('/pix/keys/x/portabilities/approve')
        .set('Authorization', token);
