...
describe('PixKeyController (e2e)', () => {
  ...
  describe('/pix/keys/id/portabilities/approve (DELETE)', () => {
    it('TC0001 - Should delete successfully', async () => {
      ...
      const res = await request(app.getHttpServer())
        .del(`/pix/keys/${pixKey.id}/portabilities/approve`)
      ...
      expect(res.body.data.state).toBe(
        KeyState.PORTABILITY_REQUEST_CANCEL_OPENED,
      );
    ...
    it('TC0002 - Should fail with invalid id', async () => {
      ...
      const res = await request(app.getHttpServer())
        .del('/pix/keys/x/portabilities/approve')
        .set('Authorization', token);
