...
import { CreatePaymentNuPayServiceKafka } from '@zro/nupay/infrastructure';
import { CreatePaymentRequest } from '@zro/nupay/interface';
import { AuthUser } from '@zro/users/domain';
import { AuthUserParam } from '@zro/users/infrastructure';

export class NuPayCreatePaymentBodyRequest {
  @ApiProperty({
...
export interface NuPayCreatePaymentResponse {
  id: string;
  status: string;
  referenceId?: string;
...
class NuPayPaymentRestResponse {
  ...
  constructor(props: NuPayCreatePaymentResponse) {
    this.id = props.id;
...
export class NuPayCreatePaymentController {
  /**
  ...
  async execute(
    ...
    @KafkaServiceParam(CreatePaymentNuPayServiceKafka)
    service: CreatePaymentNuPayServiceKafka,
    @LoggerParam(NuPayCreatePaymentController)
    logger: Logger,
    @Body() params: NuPayCreatePaymentBodyRequest,
    ...
    logger.debug('Create NuPay payment.', { user, payload });
