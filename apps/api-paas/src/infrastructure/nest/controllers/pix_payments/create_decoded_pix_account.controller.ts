...
class CreateDecodedPixAccountBody {
  @ApiProperty({
    enum: PersonType,
...
class CreateDecodedPixAccountRestResponse {
  @ApiProperty({
...
/**
 * Create decoded pix account controller. Controller is protected by JWT access token.
 */
@ApiTags('Pix | Payments')
@ApiBearerAuth()
@DefaultApiHeaders()
@TransactionApiHeader()
@Controller('pix/payment/decode/by-account')
@HasPermission('api-paas-post-pix-payments-decode-by-account')
