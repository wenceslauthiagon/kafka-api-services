...
class GetPixDepositByIdRestResponse {
  @ApiProperty({
...
export class GetPixDepositByIdRestController {
  /**
  ...
  @ApiOkResponse({
    description: 'Deposit.',
    type: GetPixDepositByIdRestResponse,
  })
  @ApiUnauthorizedResponse({
  ...
  async execute(
    ...
    @Param() params: GetPixDepositByIdParams,
    @KafkaServiceParam(GetPixDepositByIdServiceKafka)
    service: GetPixDepositByIdServiceKafka,
    @LoggerParam(GetPixDepositByIdRestController)
    logger: Logger,
  ): Promise<GetPixDepositByIdRestResponse> {
    // Create a payload.
    ...
    const response = result && new GetPixDepositByIdRestResponse(result);
