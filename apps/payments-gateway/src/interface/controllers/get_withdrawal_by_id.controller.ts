...
export class GetWithdrawalByIdController {
  constructor(
    private logger: Logger,
    readonly axiosInstance: AxiosInstance,
  ) {
    this.logger = logger.child({
      context: GetWithdrawalByIdController.name,
    });
  }
  ...
  async execute(
    ...
    this.logger.debug('Get withdrawal by id request.', { request });

    const { id } = request;

    try {
      const result = await this.axiosInstance.get<TransactionResponseItem>(
        `${PAYMENTS_GATEWAY_SERVICES.WITHDRAWAL}/${id}`,
      );
      ...
      this.logger.info('Get withdrawal by id response.', {
        response,
      });
