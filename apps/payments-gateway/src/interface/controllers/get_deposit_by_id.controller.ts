...
export class GetDepositByIdController {
  constructor(
    private logger: Logger,
    readonly axiosInstance: AxiosInstance,
  ) {
    this.logger = logger.child({
      context: GetDepositByIdController.name,
    });
  }
  ...
  async execute(
    ...
    this.logger.debug('Get deposit by id request.', { request });

    const { id } = request;

    try {
      const result = await this.axiosInstance.get<TransactionResponseItem>(
        `${PAYMENTS_GATEWAY_SERVICES.DEPOSIT}/${id}`,
      );
