...
export class GetRefundByIdController {
  constructor(
    private logger: Logger,
    readonly axiosInstance: AxiosInstance,
  ) {
    this.logger = logger.child({
      context: GetRefundByIdController.name,
    });
  }
  ...
  async execute(
    ...
    this.logger.debug('Get refund by id request.', { request });

    const { id } = request;

    try {
      const result = await this.axiosInstance.get<TransactionResponseItem>(
        `${PAYMENTS_GATEWAY_SERVICES.REFUND}/${id}`,
      );
      ...
      this.logger.info('Get refund by id response.', {
        response,
      });
