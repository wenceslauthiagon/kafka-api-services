...
async function bootstrap() {
  ...
  const sequelize = app.get(DATABASE_PROVIDER);
  ...
  app.useGlobalInterceptors(
    ...
    new TransactionInterceptor(sequelize, logger),
    new KafkaServiceInterceptor(kafkaService),
    new KafkaEventInterceptor(kafkaService),
...
function openapi() {
  const config = new DocumentBuilder()
    .setTitle('Z.ro Bank Jdpi Webhook API')
    .setDescription('Z.ro Bank Jdpi Webhook API')
