...
import {
  NotifyCreditDepositControllerEvent,
  NotifyCreditDepositEventEmitterControllerInterface,
  NotifyCreditDepositEventType,
} from '@zro/api-jdpi/interface';

const eventMapper = KAFKA_EVENTS.NOTIFY_CREDIT;

type NotifyCreditDepositKafkaEvent =
  KafkaMessage<NotifyCreditDepositControllerEvent>;

/**
 * ApiJdpi microservice.
 */
@KafkaCreateEvent(Object.values(eventMapper))
export class NotifyCreditDepositEventKafkaEmitter
  implements NotifyCreditDepositEventEmitterControllerInterface
{
  /**
   * Default constructor.
  ...
  constructor(
    ...
    this.logger = logger.child({
      context: NotifyCreditDepositEventKafkaEmitter.name,
    });
  }
  ...
  emitCreditDepositEvent(
    ...
    const data: NotifyCreditDepositKafkaEvent = {
      key: this.requestId,
