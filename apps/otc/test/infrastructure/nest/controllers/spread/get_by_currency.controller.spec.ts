...
import {
  GetSpreadByCurrencyMicroserviceController as Controller,
  SpreadDatabaseRepository,
  SpreadModel,
} from '@zro/otc/infrastructure';
import { SpreadFactory } from '@zro/test/otc/config';
import { KafkaContext } from '@nestjs/microservices';
import { GetSpreadByCurrencyRequest } from '@zro/otc/interface';

describe('GetSpreadByCurrencyMicroserviceController', () => {
  ...
  describe('GetSpreadByCurrency', () => {
    describe('With valid parameters', () => {
      it('TC0001 - Should get spread by currency successfully', async () => {
        const spread = await SpreadFactory.create<SpreadModel>(
          SpreadModel.name,
          { userId: null },
        );

        const message: GetSpreadByCurrencyRequest = {
