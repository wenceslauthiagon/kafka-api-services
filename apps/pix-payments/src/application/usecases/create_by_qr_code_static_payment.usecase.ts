...
export class CreateByQrCodeStaticPaymentUseCase {
  /**
   * Default constructor.
  ...
  constructor(
    ...
    private readonly pixPaymentOperationSendQrsTransactionTag: string,
  ) {
    this.logger = logger.child({
      context: CreateByQrCodeStaticPaymentUseCase.name,
    });
  }

  /**
   * Create qr code static Payment.
   *
  ...
  async execute(
    ...
    this.logger.debug('Receive QRCode Static Payment data.', { payment });
    ...
    // Check Sanity, Only QRCode QR_CODE_STATIC_INSTANT_PAYMENT is accept
    if (
      decodedQrCode.type !== DecodedQrCodeType.QR_CODE_STATIC_INSTANT_PAYMENT
    ) {
    ...
    const newPayment = new PaymentEntity({
      ...payment,
      paymentType: PaymentType.QR_CODE_STATIC_INSTANT,
      transactionTag: this.pixPaymentOperationSendQrsTransactionTag,
      state,
      priorityType,
      paymentDate: payment.paymentDate
    ...
    this.logger.debug('Payment by QRCode static added.', { newPayment });
