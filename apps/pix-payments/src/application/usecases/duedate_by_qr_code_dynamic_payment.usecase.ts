...
export class DuedateByQrCodeDynamicPaymentUseCase {
  /**
   * Default constructor.
  ...
  constructor(
    ...
    private readonly pixPaymentOperationDuedateQrdTransactionTag: string,
  ) {
    this.logger = logger.child({
      context: DuedateByQrCodeDynamicPaymentUseCase.name,
    });
  }

  /**
   * Duedate qr code dynamic Payment.
   *
  ...
  async execute(
    ...
    // Check Sanity, Only QRCode QR_CODE_DYNAMIC_DUE_DATE is accept
    if (decodedQrCode.type !== DecodedQrCodeType.QR_CODE_DYNAMIC_DUE_DATE) {
    ...
    payment.paymentDate = decodedQrCode.paymentDate
      ? getMoment(decodedQrCode.paymentDate).toDate()
      : getMoment().toDate();

    payment.value = decodedQrCode.paymentValue;

    // Check value qr code if greater than 0
    if (payment.value <= 0) {
    ...
    const newPayment = new PaymentEntity({
      ...payment,
      paymentType: PaymentType.QR_CODE_DYNAMIC_DUE_DATE,
      transactionTag: this.pixPaymentOperationDuedateQrdTransactionTag,
      state,
      priorityType,
    ...
    this.logger.debug('Payment by QRCode dynamic due date added.', {
      newPayment,
    });
