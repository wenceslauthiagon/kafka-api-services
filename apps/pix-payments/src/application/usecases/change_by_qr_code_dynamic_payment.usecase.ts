...
export class ChangeByQrCodeDynamicPaymentUseCase {
  /**
   * Default constructor.
  ...
  constructor(
    ...
    private readonly pixPaymentOperationChangeQrdTransactionTag: string,
  ) {
    this.logger = logger.child({
      context: ChangeByQrCodeDynamicPaymentUseCase.name,
    });
  }

  /**
   * Change qr code dynamic Payment.
   *
  ...
  async execute(
    ...
    this.logger.debug('Receive Change QRCode dynamic Payment data.', {
      payment,
    });
    ...
    // Check Sanity, Only QRCode QR_CODE_DYNAMIC_CHANGE is accept.
    if (decodedQrCode.type !== DecodedQrCodeType.QR_CODE_DYNAMIC_CHANGE) {
    ...
    // If the payment allows changing the value, use the value sent via body if greater than 0.
    if (!decodedQrCode.allowUpdate) {
      payment.value = decodedQrCode.changeValue;
    }
    ...
    // Payment value is document value + user defined value (or change value
    // if update is not allowed).
    payment.value += decodedQrCode.documentValue;

    // Set pending status and fire event pending
    const newPayment = new PaymentEntity({
      ...payment,
      paymentType: PaymentType.QR_CODE_DYNAMIC_CHANGE,
      transactionTag: this.pixPaymentOperationChangeQrdTransactionTag,
      state: PaymentState.PENDING,
      priorityType: PaymentPriorityType.PRIORITY,
      paymentDate: getMoment().toDate(),
      operation: new OperationEntity({ id: uuidV4() }),
      changeOperation: new OperationEntity({ id: uuidV4() }),
      user,
      wallet,
      ...
      agentMod: decodedQrCode.agentModChange,
      agentIspb: decodedQrCode.agentIspbChange,
    });
