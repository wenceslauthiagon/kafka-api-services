...
export class WithdrawalByQrCodeDynamicPaymentUseCase {
  /**
   * Default constructor.
  ...
  constructor(
    ...
    private readonly pixPaymentOperationWithdrawalQrdTransactionTag: string,
  ) {
    this.logger = logger.child({
      context: WithdrawalByQrCodeDynamicPaymentUseCase.name,
    });
  }

  /**
   * Withdrawal qr code dynamic Payment.
   *
  ...
  async execute(
    ...
    this.logger.debug('Receive Withdrawal QRCode dynamic Payment data.', {
      payment,
    });
    ...
    // Check Sanity, Only QRCode QR_CODE_DYNAMIC_WITHDRAWAL is accept.
    if (decodedQrCode.type !== DecodedQrCodeType.QR_CODE_DYNAMIC_WITHDRAWAL) {
    ...
    if (!decodedQrCode.allowUpdate) payment.value = decodedQrCode.withdrawValue;
    ...
    const newPayment = new PaymentEntity({
      ...payment,
      paymentType: PaymentType.QR_CODE_DYNAMIC_WITHDRAWAL,
      transactionTag: this.pixPaymentOperationWithdrawalQrdTransactionTag,
      ...
      agentIspb: decodedQrCode.agentIspbWithdrawal,
      agentMod: decodedQrCode.agentModWithdrawal,
    });
    ...
    this.logger.debug('Payment by QRCode dynamic withdrawal added.', {
      newPayment,
    });
