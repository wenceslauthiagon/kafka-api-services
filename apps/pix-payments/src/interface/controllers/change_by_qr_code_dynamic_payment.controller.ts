...
export class ChangeByQrCodeDynamicPaymentRequest
...
type TChangeByQrCodeDynamicPaymentResponse = Pick<
  Payment,
...
export class ChangeByQrCodeDynamicPaymentResponse
  extends AutoValidator
  implements TChangeByQrCodeDynamicPaymentResponse
{
  @IsUUID(4)
  id: string;
  ...
  constructor(props: TChangeByQrCodeDynamicPaymentResponse) {
    super(props);
  }
}

export class ChangeByQrCodeDynamicPaymentController {
  ...
  constructor(
    ...
    this.logger = logger.child({
      context: ChangeByQrCodeDynamicPaymentController.name,
    });
  ...
  async execute(
    request: ChangeByQrCodeDynamicPaymentRequest,
  ): Promise<ChangeByQrCodeDynamicPaymentResponse> {
    this.logger.debug('Change qr code dynamic Pix Payment request.', {
      request,
    });
    ...
    const response = new ChangeByQrCodeDynamicPaymentResponse({
      id: payment.id,
      operationId: payment.operation?.id,
      changeOperationId: payment.changeOperation?.id,
      state: payment.state,
      value: payment.value,
    ...
    this.logger.info('Change qr code dynamic Pix Payment response.', {
      payment: response,
    });
