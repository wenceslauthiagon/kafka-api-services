...
type DepositAttributes = PixDeposit & {
  userId?: string;
  walletId?: string;
  operationId?: string;
...
type DepositCreationAttributes = DepositAttributes;

@Table({
  tableName: 'pix_deposits',
  timestamps: true,
  underscored: true,
})
export class PixDepositModel
  extends DatabaseModel<DepositAttributes, DepositCreationAttributes>
  implements PixDeposit
{
  @PrimaryKey
  @AllowNull(false)
  ...
  constructor(values?: DepositAttributes, options?: BuildOptions) {
   ...
   */
  toDomain(): PixDeposit {
    const entity = new PixDepositEntity(this.get({ plain: true }));
