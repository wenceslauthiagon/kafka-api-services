...
import {
  HandleClosePendingPixInfractionReceivedEventController,
  HandleClosePendingPixInfractionReceivedEventRequest,
  HandleRevertPixInfractionEventRequest,
  PixInfractionEventEmitterControllerInterface,
} from '@zro/pix-payments/interface';

export type HandleClosePendingPixInfractionReceivedEventKafkaRequest =
  KafkaMessage<HandleClosePendingPixInfractionReceivedEventRequest>;
...
export class ClosePendingPixInfractionReceivedNestObserver {
  constructor(
   ...
   */
  @KafkaEventPattern(KAFKA_EVENTS.PIX_INFRACTION.CLOSED_PENDING_RECEIVED)
  async execute(
    @Payload('value')
    message: HandleClosePendingPixInfractionReceivedEventRequest,
    ...
    @LoggerParam(ClosePendingPixInfractionReceivedNestObserver)
    logger: Logger,
    ...
    // Parse kafka message.
    const payload = new HandleClosePendingPixInfractionReceivedEventRequest(
      message,
    );
    ...
    const controller =
      new HandleClosePendingPixInfractionReceivedEventController(
        logger,
