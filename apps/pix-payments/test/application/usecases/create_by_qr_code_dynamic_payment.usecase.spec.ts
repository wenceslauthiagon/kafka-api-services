...
describe('CreatePaymentUseCase', () => {
  ...
  const makeSut = () => {
    ...
    const transactionTag = 'PIXSENDQRD';
  ...
  describe('With invalid parameters', () => {
    ...
    it('TC0009 - Should not create if decoded qr code paymentDate is invalid', async () => {
      ...
      expect(mockCreatePaymentRepository).toHaveBeenCalledTimes(0);
      expect(mockGetOperationService).toHaveBeenCalledTimes(1);
      expect(mockGetDecodedQrCodeRepository).toHaveBeenCalledTimes(0);
    });
    ...
    it('TC0012 - Should not duedate if decoded qr code was expired', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
    ...
    it('TC0013 - Should not create if allowUpdate and value is less or equal to 0', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
  ...
  describe('With valid parameters', () => {
    it('TC0014 - Should create pending successfully when not allow update', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
      ...
      expect(result.paymentType).toBe(PaymentType.QR_CODE_DYNAMIC_INSTANT);
      expect(result.priorityType).toBe(PaymentPriorityType.PRIORITY);
      expect(result.value).toBe(decodedQrCodeValue.documentValue);
    ...
    it('TC0015 - Should create schedule successfully when allow update', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
