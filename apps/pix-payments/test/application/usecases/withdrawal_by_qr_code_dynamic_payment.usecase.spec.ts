...
describe('WithdrawalPaymentUseCase', () => {
  ...
  const makeSut = () => {
    ...
    const transactionTag = 'PIXWITHDRAWALQRD';
  ...
  describe('With invalid parameters', () => {
    it('TC0001 - Should not withdraw if missing params', async () => {
      const {
        sut,
        mockPendingPaymentEvent,
        mockGetPaymentRepository,
    ...
    it('TC0002 - Should not withdraw if payment already exists', async () => {
    ...
    it('TC0003 - Should not withdraw if payment already exists and user is forbidden', async () => {
      const user = new UserEntity({ uuid: faker.datatype.uuid() });
      const { id, decodedQrCode, value, description, wallet } =
        await PaymentFactory.create<PaymentEntity>(PaymentEntity.name);
      const payment = new PaymentEntity({
    ...
    it('TC0004 - Should not withdraw if user not found', async () => {
      const user = new UserEntity({ uuid: faker.datatype.uuid() });
      const { id, decodedQrCode, value, description, wallet } =
        await PaymentFactory.create<PaymentEntity>(PaymentEntity.name);
      const payment = new PaymentEntity({
    ...
    it('TC0005 - Should not withdraw if user found but dont has cpf', async () => {
    ...
    it('TC0006 - Should not withdraw if user found but dont has fullName', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        fullName: faker.name.fullName(),
    ...
    it('TC0007 - Should not withdraw if onboarding not found', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
    ...
    it('TC0008 - Should not withdraw if owner user wallet not found', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
    ...
    it('TC0009 - Should not withdraw if decoded qr code is not found', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
    ...
    it('TC0010 - Should not withdraw if decoded qr code is differ than READY', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
    ...
    it('TC0012 - Should not withdraw if allowUpdate and value is less or equal to 0', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
  ...
  describe('With valid parameters', () => {
    it('TC0013 - Should withdraw successfully when allow update', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
    ...
    it('TC0014 - Should withdraw successfully when dont allow update', async () => {
      const user = new UserEntity({
        id: faker.datatype.number({ min: 1, max: 99999 }),
        uuid: faker.datatype.uuid(),
        document: cpf.generate(),
      ...
      expect(result.priorityType).toBe(PaymentPriorityType.PRIORITY);
      expect(result.transactionTag).toBe(transactionTag);
      expect(result.value).toBe(decodedQrCodeValue.withdrawValue);
      expect(mockPendingPaymentEvent).toHaveBeenCalledTimes(1);
      expect(mockGetPaymentRepository).toHaveBeenCalledTimes(1);
      expect(mockCreatePaymentRepository).toHaveBeenCalledTimes(1);
      expect(mockGetOperationService).toHaveBeenCalledTimes(1);
