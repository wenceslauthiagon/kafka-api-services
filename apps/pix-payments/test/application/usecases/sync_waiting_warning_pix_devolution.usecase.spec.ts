...
import * as GetWarningPixDevolutionPspGatewayMock from '@zro/test/pix-payments/config/mocks/get_payment_by_id.mock';

describe('SyncWaitingWarningPixDevolutionUseCase', () => {
  ...
  const mockGateway = () => {
    ...
    const mockGetWarningPixDevolutionGateway: jest.Mock = On(
      warningPixDevolutionGateway,
    ).get(method((mock) => mock.getPayment));

    return {
      warningPixDevolutionGateway,
      mockGetWarningPixDevolutionGateway,
    };
  };
  ...
  const makeSut = () => {
    ...
    const { warningPixDevolutionGateway, mockGetWarningPixDevolutionGateway } =
      mockGateway();
  ...
  describe('With valid parameters', () => {
    it('TC0001 - Should emit completed event successfully when WarningPixDevolution exists and it is settled.', async () => {
      ...
      mockGetAllRepository.mockResolvedValue([warningPixDevolution]);

      mockGetWarningPixDevolutionGateway.mockResolvedValue(
        GetWarningPixDevolutionPspGatewayMock.successPaymentSettled(),
      );
    ...
    it('TC0002 - Should emit reverted event successfully when WarningPixDevolution exists and it is not settled.', async () => {
      ...
      mockGetWarningPixDevolutionGateway.mockResolvedValue(
        GetWarningPixDevolutionPspGatewayMock.successPaymentNotSettled(),
      );
      const failed = {
        errorCode: 'AB03',
        errorMessage:
    ...
    it('TC0003 - Should not emit any event when PSP does not respond.', async () => {
      ...
      mockGetWarningPixDevolutionGateway.mockResolvedValue(undefined);
    ...
    it('TC0004 - Should not emit any event when receives an error.', async () => {
      ...
      mockGetWarningPixDevolutionGateway.mockResolvedValue(
        GetWarningPixDevolutionPspGatewayMock.offline(),
      );
