...
import {
  DuedateByQrCodeDynamicPaymentMicroserviceController as Controller,
...
describe('DuedateByQrCodeDynamicPaymentMicroserviceController', () => {
  ...
  describe('DuedateByQrCodeDynamicPayment', () => {
    describe('With valid parameters', () => {
      it('TC0001 - Should duedate by qr code dynamic pending successfully', async () => {
        ...
        const decodedQrCode =
          await DecodedQrCodeFactory.create<DecodedQrCodeModel>(
            DecodedQrCodeModel.name,
            {
              ...
              paymentDate: null,
              expirationDate: getMoment().add(2, 'day').toDate(),
            },
          );
        ...
        const message: DuedateByQrCodeDynamicPaymentRequest = {
      ...
      it('TC0002 - Should duedate by qr code dynamic scheduled successfully', async () => {
        const user = new UserEntity({
          id: faker.datatype.number({ min: 1, max: 99999 }),
          uuid: faker.datatype.uuid(),
          type: PersonType.NATURAL_PERSON,
        ...
        const decodedQrCode =
          await DecodedQrCodeFactory.create<DecodedQrCodeModel>(
            DecodedQrCodeModel.name,
            {
              ...
              expirationDate: getMoment().add(5, 'day').toDate(),
            },
          );
        mockGetOnboardingService.mockResolvedValue(onboarding);
        mockGetUserService.mockResolvedValue(user);
    ...
    describe('With invalid parameters', () => {
      it('TC0003 - should dont duedate payment when user is forbidden', async () => {
        const user = new UserEntity({
          id: faker.datatype.number({ min: 1, max: 99999 }),
          uuid: faker.datatype.uuid(),
        });
        ...
        const { id, description } = await PaymentFactory.create<PaymentModel>(
          PaymentModel.name,
        );
