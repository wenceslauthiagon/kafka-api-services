...
import {
  CompletePixDevolutionNestObserver as Observer,
...
describe('CompletePixDevolutionNestObserver', () => {
  ...
  const mockEmitPixDevolutionEvent: jest.Mock = On(eventEmitter).get(
  ...
  describe('With valid parameters', () => {
    it('TC0001 - Should handle complete pixDevolution successfully', async () => {
      const { id, userId, state, walletId } =
        await PixDevolutionFactory.create<PixDevolutionModel>(
          PixDevolutionModel.name,
          { state: PixDevolutionState.WAITING },
        );

      const message: HandleCompletePixDevolutionEventRequest = {
        id,
        userId,
        walletId,
        state,
