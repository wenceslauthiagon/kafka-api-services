...
import { WarningPixDepositIsReceitaFederalNestObserver as Observer } from '@zro/pix-payments/infrastructure';
import {
...
const pixPaymentReceitaFederalCnpj = '00394460005887';

describe('WarningPixDepositIsReceitaFederalNestObserver', () => {
  ...
  describe('With invalid parameters', () => {
    it('TC0001 - Should not update pix deposit with missing params', async () => {
      const message: HandleWarningPixDepositIsReceitaFederalEventRequest = {
        id: null,
        state: null,
        userId: null,
        walletId: null,
    ...
    it('TC0002 - Should update pix deposit check successfully but do not emit event since there are other checkers to check deposit.', async () => {
      ...
      const data = await PixDepositFactory.create<PixDepositEntity>(
        PixDepositEntity.name,
        {
          ...
          amount: 50000,
          thirdPartDocument: pixPaymentReceitaFederalCnpj,
          check: checkResult,
        },
      );
