...
describe('TC056 - Payments gateway refunds', () => {
  ...
  it('UC001 - get list refunds', async () => {
    const response = await _cy.get_request(env.gateway_refunds, headers);
    ...
    _cy.expect(data, 'id'),
      ...
      _cy.expect(data, 'psp_bank_name'),
      _cy.expect(data, 'psp_ispb'),
      _cy.expect(data, 'instant_payment_id_field');
    ...
    const paid_by_client = ['id', 'company_id', 'document', 'email', 'name'];
    for (let validations of paid_by_client) {
      _cy.expect(response, validations);
    }
