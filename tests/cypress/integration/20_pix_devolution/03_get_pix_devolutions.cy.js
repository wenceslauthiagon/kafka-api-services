...
describe('TC056 - Test api pix devolutions - list users qrcode', () => {
  ...
  it('UC001 - get list user`s pix devolutions', async () => {
    const response = await _cy.get_request(env.pix_devolution, headers);
    const array = [
      'id',
      'state',
      'description',
  ...
  it('create json file', async () => {
    await _cy.write_json('pix_devolution', 'pix_devolutions', file);
  });
});
