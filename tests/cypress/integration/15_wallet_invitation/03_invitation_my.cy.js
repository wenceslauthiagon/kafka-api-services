...
describe('TC055 - Test api  Operations wallet invitations my', () => {
  let env = Env();
  let headers;
  let token;
  let file;
  ...
  it('UC001 - get list operations wallet invitation my', async () => {
    const response = await _cy.get_request(
      `${env.wallet_invitation}/my?page=1&size=20&order=asc`,
      headers,
    );
  ...
  it('create json file', async () => {
    await _cy.write_json('wallet_invitations', 'wallet_invitation_my', file);
  });
});
