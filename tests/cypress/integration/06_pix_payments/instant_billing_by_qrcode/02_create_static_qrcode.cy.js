...
describe('TC030 - Test api pix payments create QR code static', () => {
  let env = Env();
  let headers;
  let token;
  let json_file;
  ...
  before(async () => {
    json_file = await _cy.read_json('pix_payments', 'list_key_billing_qrcode');
  });
  it('UC001 - create a static qr code', async () => {
    for (let list of json_file) {
  ...
  it('UC002 -Create JSON id', async () => {
    await _cy.write_json('pix_payments', 'create_billing_qrcode', { id: id });
  });
